...
class AuthRule extends Model
{
     ...
     * @return array
     */
    public function getAuthByGroupIdToTree($UnitNo) 
    {
        $group = my_model('AuthGroup', 'model', 'admin')->getGroupById($UnitNo);
     ...
     * @return bool
     */
    public function hasAccessByName($name, $UnitNo)
    {
        ...
        $group = my_model('AuthGroup', 'model', 'admin')->getGroupById($UnitNo);
        if ($group['tree']['status'] != 1 || empty($group['tree']['rules'])) {
            return false;
        }
        $myRuleIds = explode(',', $group['tree']['rules']);
