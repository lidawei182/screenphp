...
class Admin extends Base
{
    ...
    public function index()
    {
        if ($this->request->isPost()) {
            ...
            $result['total'] = model('Admin', 'logic')
                ->getTotal(
                    $userName, 
                    $phone, 
                    $realName, 
    ...
    public function getuser()
    {
        ...
        $access  = my_model('AuthRule', 'logic', 'admin')->getAuthByGroupIdToTree($user['UnitNo']);
        
        $routers = [];
   
        foreach ($access as $v) {
            $temp = $this->getdata($v);
            foreach ($v['children'] as $k => $vo) {
                $temp['children'][] = $this->getdata($vo);
                if (array_key_exists('children', $vo)) {
                    foreach ($vo['children'] as $key => $vos) {
                      $temp['children'][$k]['children'][] = $this->getdata($vos);
                    }
                }
            }
        ...
        $user['access'] = $routers;
        $group          = my_model('AuthGroup', 'logic', 'admin')->getGroupById($user['UnitNo']);
        $user['group']  = $group['jdName'];
        $user['jdno']  = $group['jdno'];
        ajax_return_ok($user);
    }
